	...
	: PostProcessingShader(shader), Texture(), Width(width), Height(height), Confuse(GL_FALSE), Chaos(GL_FALSE), Shake(GL_FALSE)
{
	...
	/*  get glRendbufferStorageMultisample param samples value
	GLint samples;
	...
	*/
	...
	glRenderbufferStorageMultisample(GL_RENDERBUFFER, 4, GL_RGB, width, height); // Allocate storage for render buffer object(samples value 8 change to 4)
