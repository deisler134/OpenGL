...
void Game::UpdatePowerUps(GLfloat dt)
{
	...
	for( powerUpiter = PowerUps.begin(); powerUpiter != PowerUps.end(); powerUpiter++)
	{
		powerUpiter->Position += powerUpiter->Velocity * dt;
		if (powerUpiter->Activated)
		{
			powerUpiter->Duration -= dt;

			if (powerUpiter->Duration <= 0.0f)
			{
				// Remove powerup from list (will later be removed)
				powerUpiter->Activated = GL_FALSE;
				// Deactivate effects
				if (powerUpiter->Type == "sticky")
				{
				...
				else if (powerUpiter->Type == "pass-through")
				{
				...
				else if (powerUpiter->Type == "confuse")
				{
				...
				else if (powerUpiter->Type == "chaos")
				{
...
void Game::DoCollisions()
{
	...
	for ( iter = this->Levels[this->Level].Bricks.begin(); iter != this->Levels[this->Level].Bricks.end(); iter++)
	//	for (GameObject &box : this->Levels[this->Level].Bricks)
	{
		if (!iter->Destroyed)
		{
			Collision collision = CheckCollision(*Ball, *iter);
			if (std::get<0>(collision)) // If collision is true
			{
				// Destroy block if not solid
				if (!iter->IsSolid)
				{
					iter->Destroyed = GL_TRUE;
				...
				if (!(Ball->PassThrough && !iter->IsSolid)) 
	...
	for( powerUpiter = PowerUps.begin(); powerUpiter != PowerUps.end(); powerUpiter++)
	{
		if (!powerUpiter->Destroyed)
		{
			// First check if powerup passed bottom edge, if so: keep as inactive and destroy
			if (powerUpiter->Position.y >= this->Height)
				powerUpiter->Destroyed = GL_TRUE;

			if (CheckCollision(*Player, *powerUpiter))
			{	// Collided with player, now activate powerup
				ActivatePowerUp(*powerUpiter);
				powerUpiter->Destroyed = GL_TRUE;
				powerUpiter->Activated = GL_TRUE;
